...
import { ArrowLeft, Flag, MessageCircle, MoreVertical, Plus, Search, Send, ShieldOff } from 'lucide-react';
...
const Messages = () => {
  ...
  }, [activeId, invoke, loadConversations, loadMessages]);
      ...
      <Card className={`${!active ? 'hidden md:flex' : 'flex'} min-h-[calc(100vh-9rem)] flex-col`}><CardContent className="flex min-h-0 flex-1 flex-col p-0">{active ? <>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map((message) => {
          ...
          return <div key={message.id} className={`group flex items-end gap-1.5 ${mine ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${mine ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><p className={`mt-1 text-[10px] ${mine ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{when(message.created_at)}</p></div>
          </div>;
        })}<div ref={endRef} /></div>
        <div className="border-t p-3">{blockedIds.has(active.participant_id) ? <p className="px-1 text-center text-xs text-muted-foreground">You've blocked {active.participant_name}. Unblock them to send a message.</p> : <div className="flex items-end gap-2"><Textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Share a tip or strategy…" maxLength={4000} className="min-h-11 resize-none" /><Button size="icon" onClick={() => void send()} disabled={!draft.trim()} aria-label="Send message"><Send className="h-4 w-4" /></Button></div>}</div>
